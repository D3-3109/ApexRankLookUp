...
fun mapPictureMode(res: ApexResponseMap, image: ApexImage){
    ...
    //竞技场
    img = drawTextToImage(img,"街机轮换模式",80,1400,150, Color.white)
    img = drawTextToImage(img, res.ltm.current.map,80,1550,150, Color.white)
    img = drawTextToImage(img,"剩余时间："+ res.ltm.current.remainingTimer,90,1650,35, Color.white)
    img = drawTextToImage(img,"下一轮换："+ res.ltm.next.map,90,1700,35, Color.white)
...
fun mapTextMode(res:ApexResponseMap):String{
    ...
    map += "---------街机轮换模式-----------" + "\n"
    map += "当前地图:" + res.ltm.current.map + "\n"
    map += "剩余时间:" + res.ltm.current.remainingTimer + "\n"
    map += "下一轮换:" + res.ltm.next.map + "\n"
    map += "----------排位------------" + "\n"
